...
- **AI Writer + Auth + Supabase + Netlify (prototype, on branch `feature/ai-website-writer`, not yet merged to `main`):**
  ...
  - **Publishing** — root `netlify.toml` + `backend/netlify/edge-functions/render.ts` serve published sites at `kdksites.netlify.app/s/<subdomain>` (no domain needed). Publish saves config to Supabase. **Pending: user connects Netlify** (repo access + `SUPABASE_URL`/`SUPABASE_SERVICE_KEY`/`SITE_DOMAINS` env vars).
  ...
  - Full detail: [docs/DEV-LOG.md](docs/DEV-LOG.md) 2026-07-11 (Sessions 5 & 6), 2026-07-28 (Session 18).
