...
> Working understanding of the Website Builder admin panel, grounded in the actual design files in `New Design/` and the current prototype `Admin Panel/website-builder-admin-v4.html`.
...
### The four website designs: `New Design/`
...
There is also a `New Design/zenith V2/` variant and a parallel `New Design copy/` folder. `Old Designs/` holds the earlier prestige/clarity/heritage/meridian set (superseded).
...
- **Admin panel:** `Admin Panel/website-builder-admin-v4.html` — full-width editor (no preview rail), **6-step sequential flow** (see below), full-page Preview overlay with mobile/tablet/desktop toggles. Favicon = `CA India Logo.png` (also added to the four `Live/` copies; pristine samples untouched).
...
- **Sample templates are pristine and never modified.** `New Design/<template>/index.html` are the read-only samples the "Open sample" links point to. They must not contain binding code and must not be mutated by the builder.
- **Customized rendering lives in a separate copy.** Rendered/customized output goes to `Live/<template>/index.html` — a copy of the pristine sample **plus** a binding script. The admin **Preview** loads the Live copy; it never points at the sample. (In production this maps to the server rendering a per-subdomain instance; the sample template stays untouched.)
...
- **Adding a NEW template later:** (1) drop the design in `New Design/<key>/`, (2) copy it to `Live/<key>/` and append a binding `<script>` following the Apex pattern (remap selectors), (3) add one entry to the `DESIGNS` array in the admin with `path` + `live`. No other changes needed.
- **Per-user output (future):** Preview currently renders a single shared `Live/<key>` copy. Real publishing should render a per-subdomain instance (server-side, or `Live/<subdomain>/`) so users don't overwrite each other.
...
The prototype and new designs have moved ahead of `CLAUDE.md`, `backend/api-spec.md`, and `backend/database-schema.sql`:
